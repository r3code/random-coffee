# Random Coffee Cards — Техническое задание (v5.12)

**Версия:** 5.12 (актуальная спецификация реализованного продукта)
...
## Что изменилось в v5.12 (4 колоды перенесены в каталог)

### v5.12.0 — Встроенными остаются 2 колоды, остальные — в репозитории каталога

- **Встроенными остаются только `deep` и `work`**. Колоды `couples`, `first-date`, `friends`, `family` перенесены в репозиторий `r3code/random-coffee-decks` и подгружаются через каталог (секция «Каталог» в UI выбора колоды).
...
- **4 новых JSON-файла в `random-coffee-decks/decks/`**:
...
- **Линтер в `random-coffee-decks/.github/workflows/build-index.yml`** обновлён:
  - Добавлена валидация `deckCategory` (опц., slug из белого списка `couples, first-date, friendship, family, work, self, party`).
...
- 136 тестов проходят (без изменений — тесты на 4 колоды в `data/decks.js` убраны, проверяют только `deep` + `work`).
- Это позволяет добавлять новые колоды без перекомпиляции приложения — достаточно добавить JSON в репозиторий каталога.

### v5.11.0 — Категории колод + 4 новые встроенные колоды (теперь в каталоге, см. v5.12)
...
// v5.12: перенесены в репозиторий random-coffee-decks — больше не встроенные.
// Сами категории (deckCategories) остаются здесь для валидации импортируемых колод.
...
export const decks = {
  ...
  // v5.12: couples, first-date, friends, family — в random-coffee-decks/decks/
}
...
| v5.11.0 | Категории колод | `deckCategory` (7 категорий: couples, first-date, friendship, family, work, self, party); бейджи в порядке `[Категория] [Тип] [RU]`; бейдж в шапке GameView; второй ряд чипов-фильтров по категориям; валидация slug'а. |
| v5.12.0 | 4 колоды перенесены в каталог | Встроенными остаются только `deep` и `work`. Колоды `couples`, `first-date`, `friends`, `family` перенесены в репо `random-coffee-decks`, подгружаются через каталог. Линтер каталога валидирует `deckCategory` и добавляет в `index.json`. Карточки удалённых колод показывают бейдж категории. |
