# Random Coffee Cards — Техническое задание (v5.7)

**Версия:** 5.7 (актуальная спецификация реализованного продукта)
**Дата ревизии:** 2026-09-25
**Статус:** Approved — соответствует коду в репозитории `r3code/random-coffee`
**Живая страница:** https://r3code.github.io/random-coffee/

---

## Что изменилось в v5.7 (progressive disclosure для всех шагов формы)

### v5.7.0 — Progressive disclosure шагов 2/3 + QR-блок с копированием ссылки
...
- **QR-блок — ссылка над QR-кодом с кнопкой «Копировать»** — не везде есть нативный Web Share API, поэтому добавлена явная кнопка. Текст инструкции: «Покажите партнёру QR-код — он попадёт в ту же сессию. Или отправьте ссылку сообщением. Потом нажмите «Начать сессию».»
...
5. **Блок "Синхронизация с партнёром"**:
   ...
   - После выбора роли:
     ...
     - **Ссылка над QR-кодом** в моноширинной плашке (`break-all font-mono`) + кнопка `📋 Копировать` (v5.7). Не везде есть нативный Web Share API, поэтому кнопка обязательна. При копировании — визуальный feedback `✓` на 2 сек (`linkCopiedRef`), с fallback через `document.execCommand('copy')`.
...
- ℹ️ "Покажите партнёру QR-код — он попадёт в ту же сессию. Или отправьте ссылку сообщением. Потом нажмите «Начать сессию»."
- Кнопка "📋 Копировать" → feedback "✓" на 2 сек (v5.7)
- "Начать сессию ➔"
