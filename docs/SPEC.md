# Random Coffee Cards — Техническое задание (v5.3)

**Версия:** 5.3 (актуальная спецификация реализованного продукта)
**Дата ревизии:** 2026-09-22
**Статус:** Approved — соответствует коду в репозитории `r3code/random-coffee`
**Живая страница:** https://r3code.github.io/random-coffee/

---

## Что изменилось по сравнению с v4.4 (v5.0)
...
2. **Шаг 1.** Выбор колоды — **единый каталог** (v5.4) с двумя секциями, с **progressive disclosure** (v5.6):
   ...
   - **Состояние «каталог коллапсирован»** (после выбора колоды через `selectDeck`): вместо каталога показывается компактная плашка:
     - Имя выбранной колоды (`text-lg font-bold`, `line-clamp-2`).
     - Бейджи `[Встроенная|Загруженная]` + `[RU]` того же размера, что в карточке.
     - Число вопросов.
     - Кнопка `✎ Сменить` — разворачивает каталог обратно (через `expandCatalog`). `selectedDeckId` НЕ сбрасывается (текущая колода остаётся выделенной, чтобы пользователь мог сравнить с другими).
     - Плашка скрывается, каталог разворачивается (`v-show`).
...
5. **Блок "Синхронизация с партнёром"**:
   - Плейсхолдер 📷 + "↑ Выберите колоду, порядок и роль..."
   - После выбора — пометка ℹ️ "Партнёр сканирует QR-код и сразу попадает в ту же сессию. Потом нажмите «Начать сессию»."
   - QR-код + share URL
6. Кнопка "Начать сессию ➔" (disabled пока нет роли)
...
- ℹ️ "Партнёр сканирует QR-код и сразу попадает в ту же сессию. Потом нажмите «Начать сессию»."
- "Начать сессию ➔"
...
4. **Кастомные колоды** — только встроенные `deep`/`work`. Загрузка пользовательских колод — v5.1.

5. **Категории и цвет** — вопросы не размечены по категориям. — v5.3.

### Планируется в v5.1–v5.3

| Версия | Фича | Подход |
|--------|------|--------|
| v5.1.0 | Кастомные колоды | Загрузка из файла, хранение в `coffee_custom_decks`, `decks` = computed (встроенные + кастомные). `deckId` валидация `^[a-z0-9-]+$`, 3-64 символа. Конфликт → новое имя (замена запрещена). Лимит 20. |
| v5.1.0 | Полный бэкап | `exportBackup()` (все сессии + все кастомные колоды), `importBackup()` (сессии неактивные, колоды с конфликтом — спрашивать по каждой) |
| v5.2.0 | Каталог колод | `fetch()` на GitHub raw из репо `r3code/random-coffee-decks`. Простой список + поиск. Кэш 24ч. Кнопка «Проверить обновления» — только тексты, не структура. |
| v5.3.0 | Категории + цвет | Разметка `deep` (5) и `work` (5). Формат: `categories` + `categoryId` в вопросах. Цветовая полоска в GameView. |
| v5.3.0 | Шкала прогресса | 15 ячеек по `currentOrder.sequence`. Отвечен (зелёный) / Пропущен (оранжевый штрих) / Текущий (жёлтая рамка) / Будущий (серый). |
